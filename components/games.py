        ...
        for i, color in enumerate(colors):
            with cols[i]:
                if st.button(color, key=f"memory_{color}_{len(st.session_state.user_sequence)}"):
                    ...
                    user_len = len(st.session_state.user_sequence)
                    memory_len = len(st.session_state.memory_sequence)
                    
                    # Check if current input is correct so far
                    if user_len <= memory_len:
                        if st.session_state.user_sequence[user_len-1] != st.session_state.memory_sequence[user_len-1]:
                            st.session_state.game_over = True
                            st.error("❌ Wrong color! Game Over!")
                        elif user_len == memory_len:
                        ...
                        st.session_state.game_over = True
                        st.error("❌ Too many inputs! Game Over!")
